...
<main class="page-height bg-light-color">
    <div class="breadcrumb-wrapper d-flex align-items-center border-bottom">
        <h1 class="h3 mb-lg-0 mr-3 header-title mb-0" id="pageTitle">{{ trans("messages.missing-punch") }}</h1>
        ...
        <div class="collapse" id="searchFilter">
            <div class="card card-body mb-3">
                <div class="row depedent-row">
                    @if( ( session()->has('is_supervisor') && !empty(session()->get('is_supervisor')) && session()->get('is_supervisor') != false ) || ( ( session()->has('user_permission') && ( in_array(config('permission_constants.ALL_MISSING_PUNCH_REPORT'), session()->get('user_permission')  ) ) ) ) )
                ...
                <div class="table-responsive fixed-tabel-body">
                    <table class="table table-sm table-bordered text-left">
                        ...
                        <tbody class="ajax-view">
                            @include(config('constants.AJAX_VIEW_FOLDER') .'report/missing-punch-list')
						</tbody>
                    </table>
                </div>
            </div>
...
function filterData(){
	...
	searchAjax(site_url + 'filterMissingPunch' , searchFieldName);
	setTimeout(function(){

	}, )
}
var paginationUrl = site_url + 'filterMissingPunch';

function exportData(){
	var searchData = searchField();
	var export_info = {};
	export_info.url = site_url + 'filterMissingPunch';
