...
<main class="page-height bg-light-color">
    <div class="breadcrumb-wrapper d-flex align-items-center border-bottom">
        <h1 class="h3 mb-lg-0 mr-3 header-title mb-0" id="pageTitle">{{ trans("messages.form-16-report") }}</h1>
        ...
        <div class="ml-auto pt-sm-0 d-flex align-items-center">
            ...
            @endif
            <button type="button" class="btn btn-theme text-white border btn-sm button-actions-top-bar d-sm-flex align-items-center" data-toggle="collapse" data-target="#searchFilter" title="{{ trans('messages.filter') }}"><i class="fas fa-filter mr-sm-2"></i> <span class="d-sm-block d-none"> {{ trans("messages.filter") }} </span></button>
        </div>
    </div>
    <div class="container-fluid pt-3 visit-history">
        <div class="collapse" id="searchFilter">
            <div class="card card-body mb-3 depedent-row">
                <div class="row">
                @if( ( session()->has('is_supervisor') && !empty(session()->get('is_supervisor')) && session()->get('is_supervisor') != false ) || ( ( session()->has('user_permission') && ( in_array(config('permission_constants.ALL_FORM_16_REPORT'), session()->get('user_permission')  ) ) ) ) )
				...
				@endif
                    ...
                    <div class="form-group col-lg-2 col-md-3 col-sm-6 col-6">
                        <label for="search_start_date" class="control-label">{{ trans("messages.from-month") }}</label>
                        <input type="text" class="form-control" name="search_start_date" value="{{ ( isset($startMonth) ? $startMonth : '' ) }}" placeholder="{{ trans('messages.mm-yyyy') }}" autocomplete="off" />
                    </div>
                    <div class="form-group col-lg-2 col-md-3 col-sm-6 col-6">
                        <label for="search_end_date" class="control-label">{{ trans("messages.to-month") }}</label>
                        <input type="text" class="form-control" name="search_end_date" value="{{ ( isset($endMonth) ? $endMonth : '' ) }}" placeholder="{{ trans('messages.mm-yyyy') }}" autocomplete="off" />
                    </div>
        ...
        <div class="filter-result-wrapper ajax-view">
        	@include( config('constants.AJAX_VIEW_FOLDER') . 'report/form-16-report-list')
        </div>
    </div>
</main>
...
$(document).ready(function(){
	...
	$(function() {
        ...
        $("[name='search_start_date']").data('DateTimePicker').maxDate(moment('<?php echo date('Y-m-d' , strtotime($endMonth) ) ?>' , 'YYYY-MM-DD').endOf('month'));
        <?php } ?>
        <?php if( isset($startMonth) && (!empty($startMonth)) ) { ?>
    	$("[name='search_end_date']").data('DateTimePicker').minDate(moment('<?php echo date('Y-m-d' , strtotime($startMonth) ) ?>' , 'YYYY-MM-DD').startOf('month'));
    	<?php } ?>
...
var form_16_report_url = '{{config("constants.FORM_16_REPORT_URL")}}' + '/';
...
function filterData(){
	...
	searchAjax(form_16_report_url + 'form16ReportFilter' , searchFieldName);
}
var paginationUrl = form_16_report_url + 'form16ReportFilter';
pagination_view_html = 'pagination-view-html';

function exportData(){
	var searchData = searchField();
	var export_info = {};
	export_info.url = form_16_report_url + 'form16ReportFilter';
