...
<head>
    <meta charset="utf-8" />
    <title>Department Transfer Letter</title>
    @php
...
@php
    ...
    $fromDept = $data['from_department'] ?? (data_get($employee, 'departmentInfo.v_name') ?: '-');
    $toDept = $data['to_department'] ?? '-';
    $effectiveDate = isset($data['effective_date']) ? \Carbon\Carbon::parse($data['effective_date'])->format('d-M-Y') : date('d-M-Y');
    ...
    if (!$letterNo) {
        $prefix = 'DEPT';  // Using TRF as the prefix for transfer letters
        $year = date('Y');
        $month = date('m');
        
        // Get the latest sequence number for this month
        $latestLetter = DB::table('hr_letters')
            ->where('template', 'transfer')
        ...
        $letterNo = $letterNo ?? 'DEPT/' . date('Y/m') . '/001';
    }
@endphp
...
<div class="sheet">
    ...
    <div class="content-wrapper inner-page">
        ...
        <h2>Departmental Transfer Letter</h2>

        <p>Dear {{ $salutationName }},</p>

        <p>We are pleased to inform you that you are transferred from <strong>{{ $fromDept }}</strong> to <strong>{{ $toDept }}</strong>. This change will take effect from <strong>{{ $effectiveDate }}</strong>. We are confident that your skills and experience will contribute significantly to the <strong>{{ $toDept }}</strong> Department.</p>

        <p>We truly value your dedication and expertise, and we are confident that you will excel in your new role within the <strong>{{ $toDept }}</strong> Department.</p>

        <p>Thank you for your continued hard work and dedication. All other terms and conditions mentioned in your Employment Letter remain unchanged.</p>
