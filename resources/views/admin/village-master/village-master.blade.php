...
<main class="page-height bg-light-color">
    <div class="breadcrumb-wrapper d-flex align-items-center border-bottom">
        <h1 class="h3 mb-lg-0 mr-3 header-title mb-0" id="pageTitle">{{ trans("messages.village-master") }}</h1>
        <span class="head-total-counts total-record-count"></span>
        <div class="ml-auto pt-sm-0 d-flex align-items-center">
        @if(checkPermission('add_village') != false)
         <button type="button" onclick="openVillageModel(this)" class="btn btn btn-theme text-white border btn-sm mr-2 button-actions-top-bar d-sm-flex align-items-center" title="{{ trans('messages.add-village') }}"><i class="fas fa-plus mr-sm-2"></i> <span class="d-sm-block d-none"> {{ trans("messages.add-village") }}</span> </button>
        @endif
                ...
                <div class="row">
                    <div class="col-lg-3 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="search_by" class="form-control" placeholder="{{ trans('messages.search-by') }} {{ trans('messages.village-name')}}">
                        </div>
                    </div>
                    <div class="col-lg-2 col-md-6">
                        <div class="form-group">
                            <label class="control-label" for="search_city">{{ trans("messages.city") }}</label>
                            <select class="form-control" name="search_city" onchange='filterData()'>
                                <option value="">{{ trans("messages.select") }}</option>
                                @if(!empty($cityRecordDetails))
                                	@foreach($cityRecordDetails as $cityRecordDetail)
                                    	{{ $cityEncodeId = Wild_tiger::encode($cityRecordDetail->i_id); }} 
                                    	<option value='{{ $cityEncodeId }}'>{{ (!empty($cityRecordDetail->v_city_name) ? $cityRecordDetail->v_city_name : '') }}</option>
                                   	@endforeach
	                           	@endif
                            </select>
                        </div>
                    ...
                    <table class="table table-sm table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="text-left" style="min-width:150px;">{{ trans("messages.village-name") }}</th>
                                <th class="text-left" style="min-width:150px;">{{ trans("messages.city") }}</th>
                        ...
                        <tbody class='ajax-view'>
                            @include( config('constants.AJAX_VIEW_FOLDER') . 'village-master/village-master-list')
                        </tbody>
                    </table>
                </div>
            </div>
