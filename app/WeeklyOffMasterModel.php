...
use App\Models\WeeklyOffInfo;
class WeeklyOffMasterModel extends BaseModel
{
	use MySoftDeletes;
	...
	public function __construct(){
		parent::__construct();
		$this->table = config('constants.WEEKLY_OFF_MASTER_TABLE');
	}
	
	public function weeklyOffDetail(){ 
		return $this->hasOne( WeeklyOffInfo::class , 'i_weekly_off_master_id');
	}
	
	public function getRecordDetails( $where = [] , $likeData = [] ){
	
		$query = WeeklyOffMasterModel::with(['weeklyOffDetail'])->where('t_is_deleted' , 0 );
		...
		if( isset($likeData['searchBy']) && (!empty($likeData['searchBy'])) ){
			...
			$allLikeColumns = [ 'v_weekly_off_name','v_description' ];
